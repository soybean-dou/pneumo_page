...
def insert_job(user_info,job_info):
    ...
    conn = sqlite3.connect(str("pneumo_service.db"))
    c = conn.cursor()
    c.execute(f"SELECT * FROM jobs WHERE google_id='{user_info['user_id']}'")
    num=c.fetchall()
    job_num=len(num)+1
    x=dt.datetime.now()
    date=x.strftime("%A %d. %B %Y %H:%M:%S")
    c.executemany('INSERT INTO jobs VALUES (?,?, ?, ?, ?, ?,?,?)',
                  [(user_info["user_id"],user_info["username"],job_num,job_info["jobname"],job_info["filename"],"genome",state,date)])
    conn.commit()
    conn.close()
...
def read_db(user_id):
    ...
    c = conn.cursor()
    c.execute(f"SELECT * FROM jobs WHERE google_id ='{user_id}'")
    tb=c.fetchall()
    return tb
...
def update_db(username,key,state):
    ...
    c = conn.cursor()
    c.execute("UPDATE jobs SET state = ? WHERE id = ?",(state,key))
    conn.commit()
    conn.close()
